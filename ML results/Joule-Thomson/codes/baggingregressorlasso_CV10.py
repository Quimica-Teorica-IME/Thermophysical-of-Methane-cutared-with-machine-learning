...
def plot_comparison(phase_name, y_exp, y_nist, y_model, save_path=None, name=None):
    ...
    plt.scatter(y_exp, y_model, color='red', label='BaggingRegressorLasso', marker = '+', s=40, linewidth=1)

    plt.xlabel('Experimental')
    plt.ylabel('BaggingRegressorLasso & NIST')
    plt.title(phase_name)
    plt.legend()
...
#current_directory = 'gdrive/MyDrive/Thermophysical/Joule-Thomson'

current_directory = '/home/mmaximo/ML/Thermophysical/Joule-Thomson'    #if used on the cluster

## Data output
txt_header = 'Temperature (K)\tPressure (MPa)\tExperimental(K/MPa)\tNIST(K/MPa)\tModel(K/MPa)'
...
# ***** DECISION TREE *****
DT_file = check_and_get_filename(current_directory, "DecisionTree_Physical-State-Determination_BaggingRegressorLasso", "log")
...
# ********************** BaggingRegressorLasso **********************
BaggingRegressorLasso_directory = os.path.join(property_directory, "BaggingRegressorLassoCV10")
os.makedirs(BaggingRegressorLasso_directory, exist_ok=True)
BaggingRegressorLasso_file = check_and_get_filename(BaggingRegressorLasso_directory, "BaggingRegressorLasso", "log")
...
if len(X_liquid_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for liquid state with BaggingRegressorLasso
    print("Tuning hyperparameters for liquid state with BaggingRegressorLasso...")

    grid_search_liquid = GridSearchCV(
        BaggingRegressor(estimator=Lasso()),
        param_grid={
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for liquid state for Bagging with Lasso: {grid_search_liquid.best_params_}.\n", BaggingRegressorLasso_file)

    model_liquid = grid_search_liquid.best_estimator_
    model_liquid.fit(X_liquid_train, y_liquid_train)

    joblib.dump(model_liquid, os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_liquid.pkl'))

    #model_liquid_path = os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_liquid.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", BaggingRegressorLasso_file)
    calculate_metrics('liquid', y_liquid_train, y_pred_train_liquid, file=BaggingRegressorLasso_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", BaggingRegressorLasso_file)
    calculate_metrics('liquid', y_liquid_test,  y_pred_test_liquid, file=BaggingRegressorLasso_file)

    plot_overfitting_evaluation(y_liquid_test, y_pred_test_liquid, save_path=BaggingRegressorLasso_directory, name="BaggingRegressorLasso-overfitting_evaluation-liquid", phase_name="liquid")
    ...
    liquid_score_rmse = cross_val_score(model_liquid, X_liquid_Exp, y_liquid_Exp, cv=10, scoring='neg_root_mean_squared_error')
    liquid_cv_mean_rmse = np.mean(liquid_score_rmse)
    liquid_cv_std_rmse = np.std(liquid_score_rmse)

    # Mean Absolute Error (MAE)
    liquid_score_mae = cross_val_score(model_liquid, X_liquid_Exp, y_liquid_Exp, cv=10, scoring='neg_mean_absolute_error')
    liquid_cv_mean_mae = np.mean(liquid_score_mae)
    liquid_cv_std_mae = np.std(liquid_score_mae)

    # R²
    liquid_score_r2 = cross_val_score(model_liquid, X_liquid_Exp, y_liquid_Exp, cv=10, scoring='r2')
    liquid_cv_mean_r2 = np.mean(liquid_score_r2)
    liquid_cv_std_r2 = np.std(liquid_score_r2)

    # Mean Squared Error (MSE)
    liquid_score_mse = cross_val_score(model_liquid, X_liquid_Exp, y_liquid_Exp, cv=10, scoring='neg_mean_squared_error')
    liquid_cv_mean_mse = np.mean(liquid_score_mse)
    liquid_cv_std_mse = np.std(liquid_score_mse)

    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nLiquid Phase - RMSE (mean): {-liquid_cv_mean_rmse}\nRMSE (std): {liquid_cv_std_rmse}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MAE (mean): {-liquid_cv_mean_mae}\nMAE (std): {liquid_cv_std_mae}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"R² (mean): {liquid_cv_mean_r2}\nR² (std): {liquid_cv_std_r2}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MSE (mean): {-liquid_cv_mean_mse}\nMSE (std): {liquid_cv_std_mse}", BaggingRegressorLasso_file)

    #liquid
    save_to_file_and_print(">>> Comparison: Liquid <<<\n", BaggingRegressorLasso_file)
    calculate_metrics("liquid - Exp x NIST", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=BaggingRegressorLasso_file)
    calculate_metrics("liquid - Exp x Model", y_liquid_Exp_array, y_pred_liquid_cv_array, file=BaggingRegressorLasso_file)
    calculate_metrics("liquid - NIST x Model", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=BaggingRegressorLasso_file)

    plot_comparison("liquid", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNISTxExp-liquid")
    plot_overfitting_evaluation(y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNIST-liquid", phase_name='Liquid')


    # Save liquid data
    liquid_data = np.column_stack((X_liquid_Exp, y_liquid_Exp_array, y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array))
    np.savetxt(os.path.join(BaggingRegressorLasso_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header=txt_header)
else:
...
if len(X_vapor_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for vapor state with BaggingRegressorLasso
    print("Tuning hyperparameters for vapor state with BaggingRegressorLasso...")
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for vapor state for Bagging with Lasso: {grid_search_vapor.best_params_}.\n", BaggingRegressorLasso_file)

    model_vapor = grid_search_vapor.best_estimator_
    model_vapor.fit(X_vapor_train, y_vapor_train)

    joblib.dump(model_vapor, os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_vapor.pkl'))

    #model_vapor_path = os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_vapor.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", BaggingRegressorLasso_file)
    calculate_metrics('vapor', y_vapor_train, y_pred_train_vapor, file=BaggingRegressorLasso_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", BaggingRegressorLasso_file)
    calculate_metrics('vapor', y_vapor_test,  y_pred_test_vapor, file=BaggingRegressorLasso_file)

    plot_overfitting_evaluation(y_vapor_test, y_pred_test_vapor, save_path=BaggingRegressorLasso_directory, name="BaggingRegressorLasso-overfitting_evaluation-vapor", phase_name="vapor")
    ...
    # --------- Gridsearch metrics --------- 
    # Neg Root Mean Squared Error (-RMSE)
    vapor_score_rmse = cross_val_score(model_vapor, X_vapor_Exp, y_vapor_Exp, cv=10, scoring='neg_root_mean_squared_error')
    vapor_cv_mean_rmse = np.mean(vapor_score_rmse)
    vapor_cv_std_rmse = np.std(vapor_score_rmse)

    # Mean Absolute Error (MAE)
    vapor_score_mae = cross_val_score(model_vapor, X_vapor_Exp, y_vapor_Exp, cv=10, scoring='neg_mean_absolute_error')
    vapor_cv_mean_mae = np.mean(vapor_score_mae)
    vapor_cv_std_mae = np.std(vapor_score_mae)

    # R²
    vapor_score_r2 = cross_val_score(model_vapor, X_vapor_Exp, y_vapor_Exp, cv=10, scoring='r2')
    vapor_cv_mean_r2 = np.mean(vapor_score_r2)
    vapor_cv_std_r2 = np.std(vapor_score_r2)

    # Mean Squared Error (MSE)
    vapor_score_mse = cross_val_score(model_vapor, X_vapor_Exp, y_vapor_Exp, cv=10, scoring='neg_mean_squared_error')
    vapor_cv_mean_mse = np.mean(vapor_score_mse)
    vapor_cv_std_mse = np.std(vapor_score_mse)

    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nVapor Phase - RMSE (mean): {-vapor_cv_mean_rmse}\nRMSE (std): {vapor_cv_std_rmse}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MAE (mean): {-vapor_cv_mean_mae}\nMAE (std): {vapor_cv_std_mae}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"R² (mean): {vapor_cv_mean_r2}\nR² (std): {vapor_cv_std_r2}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MSE (mean): {-vapor_cv_mean_mse}\nMSE (std): {vapor_cv_std_mse}", BaggingRegressorLasso_file)



    #vapor
    save_to_file_and_print(">>> Comparison: Vapor <<<\n", BaggingRegressorLasso_file)
    calculate_metrics("vapor - Exp x NIST", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=BaggingRegressorLasso_file)
    calculate_metrics("vapor - Exp x Model", y_vapor_Exp_array, y_pred_vapor_cv_array, file=BaggingRegressorLasso_file)
    calculate_metrics("vapor - NIST x Model", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=BaggingRegressorLasso_file)

    plot_comparison("vapor",  y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNISTxExp-vapor")
    plot_overfitting_evaluation(y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNIST-vapor", phase_name='Vapor')

    # Save vapor data
    vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
    np.savetxt(os.path.join(BaggingRegressorLasso_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header=txt_header)
else:
...
if len(X_supercritical_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for supercritical state with BaggingRegressorLasso
    print("Tuning hyperparameters for supercritical state with BaggingRegressorLasso...")
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for supercritical state for Bagging with Lasso: {grid_search_supercritical.best_params_}.\n", BaggingRegressorLasso_file)
    ...
    joblib.dump(model_supercritical, os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_supercritical.pkl'))

    #model_supercritical_path = os.path.join(BaggingRegressorLasso_directory, 'Model_BaggingRegressorLasso_supercritical.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", BaggingRegressorLasso_file)
    calculate_metrics('supercritical', y_supercritical_train, y_pred_train_supercritical, file=BaggingRegressorLasso_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", BaggingRegressorLasso_file)
    calculate_metrics('supercritical', y_supercritical_test,  y_pred_test_supercritical, file=BaggingRegressorLasso_file)

    plot_overfitting_evaluation(y_supercritical_test, y_pred_test_supercritical, save_path=BaggingRegressorLasso_directory, name="BaggingRegressorLasso-overfitting_evaluation-supercritical", phase_name="supercritical")
    ...
    # --------- Gridsearch metrics --------- 
    # Neg Root Mean Squared Error (-RMSE)
    supercritical_score_rmse = cross_val_score(model_supercritical, X_supercritical_Exp, y_supercritical_Exp, cv=10, scoring='neg_root_mean_squared_error')
    supercritical_cv_mean_rmse = np.mean(supercritical_score_rmse)
    supercritical_cv_std_rmse = np.std(supercritical_score_rmse)

    # Mean Absolute Error (MAE)
    supercritical_score_mae = cross_val_score(model_supercritical, X_supercritical_Exp, y_supercritical_Exp, cv=10, scoring='neg_mean_absolute_error')
    supercritical_cv_mean_mae = np.mean(supercritical_score_mae)
    supercritical_cv_std_mae = np.std(supercritical_score_mae)

    # R²
    supercritical_score_r2 = cross_val_score(model_supercritical, X_supercritical_Exp, y_supercritical_Exp, cv=10, scoring='r2')
    supercritical_cv_mean_r2 = np.mean(supercritical_score_r2)
    supercritical_cv_std_r2 = np.std(supercritical_score_r2)

    # Mean Squared Error (MSE)
    supercritical_score_mse = cross_val_score(model_supercritical, X_supercritical_Exp, y_supercritical_Exp, cv=10, scoring='neg_mean_squared_error')
    supercritical_cv_mean_mse = np.mean(supercritical_score_mse)
    supercritical_cv_std_mse = np.std(supercritical_score_mse)

    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nSupercritical Phase - RMSE (mean): {-supercritical_cv_mean_rmse}\nRMSE (std): {supercritical_cv_std_rmse}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MAE (mean): {-supercritical_cv_mean_mae}\nMAE (std): {supercritical_cv_std_mae}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"R² (mean): {supercritical_cv_mean_r2}\nR² (std): {supercritical_cv_std_r2}", BaggingRegressorLasso_file)
    save_to_file_and_print(f"MSE (mean): {-supercritical_cv_mean_mse}\nMSE (std): {supercritical_cv_std_mse}", BaggingRegressorLasso_file)


    #supercritical
    save_to_file_and_print(">>> Comparison: Supercritical <<<\n", BaggingRegressorLasso_file)
    calculate_metrics("supercritical - Exp x NIST", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=BaggingRegressorLasso_file)
    calculate_metrics("supercritical - Exp x Model", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=BaggingRegressorLasso_file)
    calculate_metrics("supercritical - NIST x Model", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=BaggingRegressorLasso_file)

    plot_comparison("supercritical", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNISTxExp-supercritical")
    plot_overfitting_evaluation(y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=BaggingRegressorLasso_directory, name="ModelxNIST-Supercritical", phase_name='Supercritical')
    ...
    np.savetxt(os.path.join(BaggingRegressorLasso_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header=txt_header)
else:
    save_to_file_and_print(f"!!!!! No data points available for the supercritical state for the {property_name} !!!!!", BaggingRegressorLasso_file)
