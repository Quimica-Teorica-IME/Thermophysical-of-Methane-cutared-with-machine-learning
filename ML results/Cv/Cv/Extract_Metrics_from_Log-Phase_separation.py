...
def write_metrics_to_csv_global(csv_writer, log_file_name, directory_name, phrases, lines):
    ...
    csv_writer.writerow([f" ======== {directory_name} ========"])
    for state in states:
...
def write_metrics_to_csv_for_phase(csv_writer, phase, log_file_name, directory_name, phrases, lines):
    csv_writer.writerow([f" ======== {directory_name} ========"])
    csv_writer.writerow(['Comparison', 'MAPE', 'MAE', 'MSE', 'RMSE', 'R2'])
    for phrase in phrases:
        if phase in phrase:
            for i, line in enumerate(lines):
                if phrase.lower().strip() in line.lower().strip():
                    ...
                    formatted_metrics = format_metrics(metrics)
                    metric_values = [formatted_metrics.get(name, '') for name in ['MAPE', 'MAE', 'MSE', 'RMSE', 'R2']]
                    comparison = phrase.replace(f"{phase} - ", "")
                    csv_writer.writerow([comparison] + metric_values)
    csv_writer.writerow([])  # extra line between phases
...
def main():
    ...
    # Create separate CSV files for each phase
    output_liquid = os.path.join(current_directory, "results-liquid.csv")
    output_vapor = os.path.join(current_directory, "results-vapor.csv")
    output_supercritical = os.path.join(current_directory, "results-supercritical.csv")

    # Open each file for writing
    with open(output_liquid, mode='w', newline='', encoding='utf-8') as liquid_file, \
         open(output_vapor, mode='w', newline='', encoding='utf-8') as vapor_file, \
         open(output_supercritical, mode='w', newline='', encoding='utf-8') as supercritical_file:

        liquid_writer = csv.writer(liquid_file)
        vapor_writer = csv.writer(vapor_file)
        supercritical_writer = csv.writer(supercritical_file)
        ...
        for subdir in os.listdir(current_directory):
            ...
            # Define the phrases to search for in the log file
            ...
            # Write metrics to the appropriate phase-specific CSV file
            for phase in ['liquid', 'vapor', 'supercritical']:
                if phase == 'liquid':
                    write_metrics_to_csv_for_phase(liquid_writer, phase, os.path.splitext(log_file)[0], subdir, phrases, lines)
                elif phase == 'vapor':
                    write_metrics_to_csv_for_phase(vapor_writer, phase, os.path.splitext(log_file)[0], subdir, phrases, lines)
                elif phase == 'supercritical':
                    write_metrics_to_csv_for_phase(supercritical_writer, phase, os.path.splitext(log_file)[0], subdir, phrases, lines)

    print("\n✅ Results saved to:")
    print(f" - {output_liquid}")
    print(f" - {output_vapor}")
    print(f" - {output_supercritical}")
