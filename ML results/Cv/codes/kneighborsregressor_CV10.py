...
def plot_comparison(phase_name, y_exp, y_nist, y_model, save_path=None, name=None):
    ...
    plt.scatter(y_exp, y_model, color='red', label='KNeighbors', marker = '+', s=40, linewidth=1)

    plt.xlabel('Experimental')
    plt.ylabel('KNeighbors & NIST')
    plt.title(phase_name)
    plt.legend()
...
# ***** DECISION TREE *****
DT_file = check_and_get_filename(current_directory, "DecisionTree_Physical-State-Determination_KNeighbors", "log")
...
# ********************** KNeighbors **********************
KNeighbors_directory = os.path.join(property_directory, "KNeighbors")
os.makedirs(KNeighbors_directory, exist_ok=True)
KNeighbors_file = check_and_get_filename(KNeighbors_directory, "KNeighbors", "log")
...
'''
p defines the power parameter for the Minkowski distance metric. The Minkowski distance generalizes several other distance metrics:

When p = 1, the distance metric becomes the Manhattan distance (also known as L1 norm or city block distance).
It is the sum of the absolute differences between the coordinates of the points.
Manhattan distance = ∑∣𝑥𝑖−𝑦𝑖∣

When p = 2, the distance metric becomes the Euclidean distance (also known as L2 norm). It is the straight-line distance between two points in Euclidean space.
Euclidean distance = [∑(xi-yi)^2]^1/2

For other values of p, the distance is computed as:
Minkowski distance p = (∑∣𝑥𝑖−𝑦𝑖∣^p)1/p
'''
...
if len(X_liquid_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for liquid state with KNeighborsRegressor
    print("Tuning hyperparameters for liquid state with KNeighborsRegressor...")
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for liquid state for KNeighbors: {grid_search_liquid.best_params_}.\n", KNeighbors_file)

    model_liquid = grid_search_liquid.best_estimator_
    model_liquid.fit(X_liquid_train, y_liquid_train)

    joblib.dump(model_liquid, os.path.join(KNeighbors_directory, 'Model_KNeighbors_liquid.pkl'))

    #model_liquid_path = os.path.join(KNeighbors_directory, 'Model_KNeighbors_liquid.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", KNeighbors_file)
    calculate_metrics('liquid', y_liquid_train, y_pred_train_liquid, file=KNeighbors_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", KNeighbors_file)
    calculate_metrics('liquid', y_liquid_test,  y_pred_test_liquid, file=KNeighbors_file)

    plot_overfitting_evaluation(y_liquid_test, y_pred_test_liquid, save_path=KNeighbors_directory, name="KNeighbors-overfitting_evaluation-liquid", phase_name="liquid")
    ...
    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nLiquid Phase - RMSE (mean): {-liquid_cv_mean_rmse}\nRMSE (std): {liquid_cv_std_rmse}", KNeighbors_file)
    save_to_file_and_print(f"MAE (mean): {-liquid_cv_mean_mae}\nMAE (std): {liquid_cv_std_mae}", KNeighbors_file)
    save_to_file_and_print(f"R² (mean): {liquid_cv_mean_r2}\nR² (std): {liquid_cv_std_r2}", KNeighbors_file)
    save_to_file_and_print(f"MSE (mean): {-liquid_cv_mean_mse}\nMSE (std): {liquid_cv_std_mse}", KNeighbors_file)

    #liquid
    save_to_file_and_print(">>> Comparison: Liquid <<<\n", KNeighbors_file)
    calculate_metrics("liquid - Exp x NIST", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=KNeighbors_file)
    calculate_metrics("liquid - Exp x Model", y_liquid_Exp_array, y_pred_liquid_cv_array, file=KNeighbors_file)
    calculate_metrics("liquid - NIST x Model", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=KNeighbors_file)

    plot_comparison("liquid", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=KNeighbors_directory, name="ModelxNISTxExp-liquid")
    plot_overfitting_evaluation(y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=KNeighbors_directory, name="ModelxNIST-liquid", phase_name='Liquid')


    # Save liquid data
    liquid_data = np.column_stack((X_liquid_Exp, y_liquid_Exp_array, y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array))
    np.savetxt(os.path.join(KNeighbors_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header=txt_header)
else:
...
if len(X_vapor_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for vapor state with KNeighborsRegressor

    print("Tuning hyperparameters for vapor state with KNeighborsRegressor...")
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for vapor state for KNeighbors: {grid_search_vapor.best_params_}.\n", KNeighbors_file)

    model_vapor = grid_search_vapor.best_estimator_
    model_vapor.fit(X_vapor_train, y_vapor_train)

    joblib.dump(model_vapor, os.path.join(KNeighbors_directory, 'Model_KNeighbors_vapor.pkl'))

    #model_vapor_path = os.path.join(KNeighbors_directory, 'Model_KNeighbors_vapor.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", KNeighbors_file)
    calculate_metrics('vapor', y_vapor_train, y_pred_train_vapor, file=KNeighbors_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", KNeighbors_file)
    calculate_metrics('vapor', y_vapor_test,  y_pred_test_vapor, file=KNeighbors_file)

    plot_overfitting_evaluation(y_vapor_test, y_pred_test_vapor, save_path=KNeighbors_directory, name="KNeighbors-overfitting_evaluation-vapor", phase_name="vapor")
    ...
    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nVapor Phase - RMSE (mean): {-vapor_cv_mean_rmse}\nRMSE (std): {vapor_cv_std_rmse}", KNeighbors_file)
    save_to_file_and_print(f"MAE (mean): {-vapor_cv_mean_mae}\nMAE (std): {vapor_cv_std_mae}", KNeighbors_file)
    save_to_file_and_print(f"R² (mean): {vapor_cv_mean_r2}\nR² (std): {vapor_cv_std_r2}", KNeighbors_file)
    save_to_file_and_print(f"MSE (mean): {-vapor_cv_mean_mse}\nMSE (std): {vapor_cv_std_mse}", KNeighbors_file)



    #vapor
    save_to_file_and_print(">>> Comparison: Vapor <<<\n", KNeighbors_file)
    calculate_metrics("vapor - Exp x NIST", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=KNeighbors_file)
    calculate_metrics("vapor - Exp x Model", y_vapor_Exp_array, y_pred_vapor_cv_array, file=KNeighbors_file)
    calculate_metrics("vapor - NIST x Model", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=KNeighbors_file)

    plot_comparison("vapor",  y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=KNeighbors_directory, name="ModelxNISTxExp-vapor")
    plot_overfitting_evaluation(y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=KNeighbors_directory, name="ModelxNIST-vapor", phase_name='Vapor')

    # Save vapor data
    vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
    np.savetxt(os.path.join(KNeighbors_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header=txt_header)
else:
...
if len(X_supercritical_Exp) > 0:
    ...
    # Grid Search for Hyperparameter Tuning for supercritical state with KNeighborsRegressor
    print("Tuning hyperparameters for supercritical state with KNeighborsRegressor...")

    param_grid_supercritical = {
        'n_neighbors': [3, 5, 7, 9],
        'weights': ['uniform', 'distance'],
        'p': [1, 2],
        'algorithm': ['auto', 'ball_tree', 'kd_tree', 'brute'],
    ...
    grid_search_supercritical = GridSearchCV(
        estimator=KNeighborsRegressor(),
        param_grid=param_grid_supercritical,
        cv=10,
        scoring='neg_mean_squared_error',
        verbose=0,
        n_jobs=-1
    ...
    save_to_file_and_print(f"\nBest Hyperparameters for supercritical state for KNeighbors: {grid_search_supercritical.best_params_}.\n", KNeighbors_file)
    ...
    joblib.dump(model_supercritical, os.path.join(KNeighbors_directory, 'Model_KNeighbors_supercritical.pkl'))

    #model_supercritical_path = os.path.join(KNeighbors_directory, 'Model_KNeighbors_supercritical.pkl')
    ...
    # >>> Train <<<
    save_to_file_and_print("\n > Train data <", KNeighbors_file)
    calculate_metrics('supercritical', y_supercritical_train, y_pred_train_supercritical, file=KNeighbors_file)

    # >>> Test <<<
    save_to_file_and_print("\n > Test data <", KNeighbors_file)
    calculate_metrics('supercritical', y_supercritical_test,  y_pred_test_supercritical, file=KNeighbors_file)

    plot_overfitting_evaluation(y_supercritical_test, y_pred_test_supercritical, save_path=KNeighbors_directory, name="KNeighbors-overfitting_evaluation-supercritical", phase_name="supercritical")
    ...
    save_to_file_and_print(f"\n*** Cross-Validation Metrics ***\nSupercritical Phase - RMSE (mean): {-supercritical_cv_mean_rmse}\nRMSE (std): {supercritical_cv_std_rmse}", KNeighbors_file)
    save_to_file_and_print(f"MAE (mean): {-supercritical_cv_mean_mae}\nMAE (std): {supercritical_cv_std_mae}", KNeighbors_file)
    save_to_file_and_print(f"R² (mean): {supercritical_cv_mean_r2}\nR² (std): {supercritical_cv_std_r2}", KNeighbors_file)
    save_to_file_and_print(f"MSE (mean): {-supercritical_cv_mean_mse}\nMSE (std): {supercritical_cv_std_mse}", KNeighbors_file)


    #supercritical
    save_to_file_and_print(">>> Comparison: Supercritical <<<\n", KNeighbors_file)
    calculate_metrics("supercritical - Exp x NIST", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=KNeighbors_file)
    calculate_metrics("supercritical - Exp x Model", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=KNeighbors_file)
    calculate_metrics("supercritical - NIST x Model", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=KNeighbors_file)

    plot_comparison("supercritical", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=KNeighbors_directory, name="ModelxNISTxExp-supercritical")
    plot_overfitting_evaluation(y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=KNeighbors_directory, name="ModelxNIST-Supercritical", phase_name='Supercritical')
    ...
    np.savetxt(os.path.join(KNeighbors_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header=txt_header)
else:
    save_to_file_and_print(f"!!!!! No data points available for the supercritical state for the {property_name} !!!!!", KNeighbors_file)